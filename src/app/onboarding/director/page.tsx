...
import { ChevronLeft, CheckCircle2, User, Upload } from "lucide-react";
...
const STYLES = [
  { id: "INTENSE", label: "INTENSE DRAMA", bg: "https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=1000&auto=format&fit=crop" },
  { id: "COMMERCIAL", label: "COMMERCIAL", bg: "https://images.unsplash.com/photo-1517604931442-7e0c8ed2963c?q=80&w=1000&auto=format&fit=crop" },
  { id: "THRILLER", label: "THRILLER", bg: "https://images.unsplash.com/photo-1509248961158-e54f6934749c?q=80&w=1000&auto=format&fit=crop" },
  { id: "REALISM", label: "HYPER-REALISM", bg: "https://images.unsplash.com/photo-1518133910546-b6c2fb7d79e3?q=80&w=1000&auto=format&fit=crop" },
  { id: "ART_HOUSE", label: "ART HOUSE", bg: "https://images.unsplash.com/photo-1485846234645-a62644f84728?q=80&w=1000&auto=format&fit=crop" },
  { id: "ACTION", label: "ACTION", bg: "https://images.unsplash.com/photo-1533928298208-27ff66555d8d?q=80&w=1000&auto=format&fit=crop" },
  { id: "PERIOD", label: "PERIOD DRAMA", bg: "https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?q=80&w=1000&auto=format&fit=crop" },
  { id: "SATIRE", label: "SATIRE", bg: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=1000&auto=format&fit=crop" },
];
const AVAILABILITY_LABELS = ["ACTIVE PROJECTS", "PLANNING PHASE", "RESEARCHING"];

export default function DirectorOnboardingFlow() {
  ...
  const [otherVision, setOtherVision] = useState("");
  ...
  const fieldsCompleted = useMemo(() => {
    ...
    if (styles.length > 0) count++;
    if (previewUrl) count++;
    if (experience) count++;
    if (availability !== null) count++;
    if (locationValue) count++;
    return count;
  }, [vision, languages, styles, previewUrl, experience, availability, locationValue]);
      ...
      <div className="flex-1 flex flex-col justify-center max-w-5xl mx-auto w-full px-6 py-20 mt-8 relative z-10">
        <AnimatePresence mode="wait">
          ...
          {STEPS[stepIndex] === "WELCOME" && (
            ...
            >
              <h1 className="text-6xl md:text-9xl font-black uppercase tracking-tighter leading-none mb-8">
                DIRECTOR<br /><span className="gradient-text-blood">ONBOARDING</span>
              </h1>
              ...
              <div className="border-l-4 border-[#ff1a1a] pl-6 py-2 mb-12">
                <p className="text-zinc-500 font-bold uppercase tracking-widest text-lg md:text-xl italic">
                  "Build your vision. Find your talent."
                </p>
                <p className="text-white font-black uppercase tracking-[0.3em] text-sm md:text-base mt-4">
                  Initialize your director mainframe in under 60 seconds
                </p>
              </div>
          ...
          {STEPS[stepIndex] === "VISION" && (
            ...
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                WHAT IS YOUR<br /><span className="gradient-text-blood">PRIMARY GOAL?</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                Define your current objective. We’ll surface talent and tools aligned with your specific creative mission.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {["FIND NEW TALENT", "CASTING PROJECTS", "BUILD NETWORK", "CREATIVE OPS", "SCOUTING", "BRANDING", "COLLABORATIONS", "OTHERS"].map((opt) => (
                  <button
                    key={opt}
                    onClick={() => { 
                      if (opt === "OTHERS") {
                    ...
                    className={`p-10 glass-panel-premium border border-[#ff1a1a]/30 shadow-[0_0_15px_rgba(255,26,26,0.1)] rounded-3xl text-left hover:border-[#ff1a1a] transition-all duration-300 group cursor-pointer ${vision === opt ? 'bg-[#8a0303]/20 border-[#ff1a1a]' : ''}`}
                  >
                ...
                {showOtherInput && (
                  ...
                  >
                    <input 
                      ...
                      value={otherVision}
                      onChange={(e) => setOtherVision(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && otherVision && nextStep()}
                    />
                    <button 
                      disabled={!otherVision}
                      onClick={nextStep}
          ...
          {STEPS[stepIndex] === "LANGUAGE" && (
            ...
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                PREFFERED FILM<br /><span className="gradient-text-blood">LANGUAGES?</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                Select the languages you typically work with. This helps us filter talent with appropriate linguistic mastery for your projects.
              </p>
          ...
          {STEPS[stepIndex] === "STLYE" && (
            ...
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                YOUR DIRECTORIAL<br /><span className="gradient-text-blood">SIGNATURE?</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                Select the creative styles that define your work. This helps us align your dashboard with relevant industry insights and talent pools.
              </p>
              
              <div className="flex-1 w-full overflow-x-auto snap-x snap-mandatory flex md:grid md:grid-cols-4 gap-6 pb-12 hide-scrollbar">
                {STYLES.map((opt) => {
                  const isSelected = styles.includes(opt.label);
                  const isMax = styles.length >= 3;
                  return (
                    <button
                      key={opt.id}
                      onClick={() => {
                        if (isSelected) {
                          setStyles(prev => prev.filter(p => p !== opt.label));
                        } else if (!isMax) {
                          setStyles(prev => [...prev, opt.label]);
                        }
                      }}
              ...
              <button
                ...
                className="mt-8 px-10 py-6 bg-white text-black font-black text-2xl uppercase tracking-tighter disabled:opacity-30 hover:bg-gradient-to-r from-[#ff1a1a] to-[#8a0303] hover:text-white transition-all border border-white/10 rounded-3xl cursor-pointer"
              >
                CONFIRM SIGNATURE ({styles.length}/3)
              </button>
            </motion.div>
          )}
          ...
          {STEPS[stepIndex] === "IDENTITY" && (
            ...
            >
              <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-none">
                ESTABLISH YOUR<br /><span className="gradient-text-blood">DIRECTOR IDENTITY</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                Upload a professional headshot. Your identity is your brand in the MM8 ecosystem.
              </p>
                  ...
                  <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer group">
                    ...
                    {previewUrl ? (
                      <div className="relative w-full h-full">
                        ...
                        >
                          <span className="text-white font-black uppercase text-xs tracking-widest">ADJUST IMAGE</span>
                        </div>
                      </div>
                    ) : (
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                >
                  {uploading || isUploadingBackground ? "PROCESSING..." : "FINALIZE IDENTITY"}
                </button>
                {file && !uploading && (
                  <button
                    type="button"
          ...
          {STEPS[stepIndex] === "EXPERIENCE" && (
            ...
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                WHAT IS YOUR<br /><span className="gradient-text-blood">EXPERIENCE LEVEL?</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                Tell us about your journey. Whether you're an established professional or an emerging voice, we have a place for you.
              </p>
              <div className="flex flex-col gap-6">
                {["PRO DIRECTOR", "EMERGING VOICE", "INDEPENDENT", "STUDENT / NEW"].map((opt) => (
                  <button
                    key={opt}
          ...
          {STEPS[stepIndex] === "AVAILABILITY" && (
            ...
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                PROJECT<br /><span className="gradient-text-blood">STATUS?</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                What are you currently focusing on? This helps us prioritize your casting and networking feeds.
              </p>
          ...
          {STEPS[stepIndex] === "LOCATION" && (
            ...
            >
              <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none">
                FINAL<br /><span className="gradient-text-blood">INITIALIZATION</span>
              </h2>
              <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs max-w-2xl border-l-4 border-brand-red-dark pl-4">
                Almost there. Set your primary operational base and tell us how you discovered the MM8 network.
              </p>
              ...
              <div className="flex flex-col gap-4 mt-8">
                <label className="text-zinc-500 font-black uppercase tracking-widest text-xs">Discovery Source</label>
                <input 
                  type="text" 
                  value={acquisition}
                  onChange={(e) => setAcquisition(e.target.value)}
                  placeholder="INSTAGRAM, FRIEND, INDUSTRY AGENT, ETC." 
            ...
            >
              ...
              <div>
                <h2 className="text-6xl md:text-9xl font-black uppercase tracking-tighter leading-none mb-6">
                  MAINFRAME<br /><span className="gradient-text-blood">SECURED</span>
                </h2>
                
                <div className="flex flex-col items-center gap-4 mt-8">
                  <div className="flex items-baseline gap-4">
                    <span className="text-zinc-500 font-black uppercase tracking-widest text-xl">NETWORK STRENGTH</span>
